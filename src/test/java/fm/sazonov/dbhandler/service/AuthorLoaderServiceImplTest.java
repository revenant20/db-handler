...
class AuthorLoaderServiceImplTest {

    @Autowired
    AuthorLoaderService authorLoaderService;
    ...
    @Transactional
    void testLoad() {
        authorLoaderService.load();

        var tolkien = authorRepository.findAuthorByName("Tolkien");
        assertNotNull(tolkien);

        var books = tolkien.getBooks();
        ...
        for (var book : books) {
            ...
            assertEquals(tolkien.getId(), author.getId());
        }
